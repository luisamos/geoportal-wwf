...
<div class="admin-layer--content">
    ...
    <div class="row flex g-10 justify-right mr-bottom-10">
        <button class="btn btn--medium bg-blue" data-fancybox data-src="#modalFormServicioGeografico"
            onclick="openModal()">
                    ...
                    <tr>
                        <td>
                            <div class="selected-article">
                                <label>
                                    <input type="radio" name="ckServicioGeografico"
                                        data-id="<?=$arrRegistros[$p]['id_servicio_geografico']?>"><span>&nbsp;</span>
                                </label>
                            </div>
                        </td>
                        <td><?=$arrRegistros[$p]['id_servicio_geografico']?></td>
                        <td><?=$arrRegistros[$p]['tipo']?></td>
                        <td><?=$arrRegistros[$p]['descripcion_tematica']?></td>
                        <td><?=$arrRegistros[$p]['alias']?></td>
                        <td>
                            <?php
                            $url = $arrRegistros[$p]['direccion_web'];
                            $host = parse_url($url, PHP_URL_HOST);
                            $parts = explode('.', $host);
                            $institucion = strtoupper($parts[1]);
                            echo $institucion;
                        ?>
                        </td>
            ...
            <div class="footer-pagination">
                ...
                <div class="pagination-datatable">
                    <a class="button-small-arrow arrow-left"
                        href="<?= ($data['pagina']==1) ? "#" : base_url()."serviciosgeograficos/page/".($data['pagina']-1) ?>">
                    ...
                    <a class="button-small-arrow arrow-right"
                        href="<?= base_url() ?>serviciosgeograficos/page/<?= ($data['pagina']< $data['total_paginas']) ? $data['pagina']+1 : $data['total_paginas'] ?>"><svg
...
<?php getModal('modalServiciosGeografico',$data);?>
...
$('#aServiciosGeograficos').addClass('active');
</script>