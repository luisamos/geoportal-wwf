      ...
      <div class="row"> 
        <div class="tabs">
          <div class="x-tab right" role="tablist" aria-label="">
            ...
            ?>
            <a href="<?= base_url() ?>/archivoscampo/tipo/<?=$arrTipos[$p]['id_tipo_archivo']?>/1">
              <button id="tab-lastnews-<?=($p+1)?>" 
                role="tab" 
                aria-selected="<?= ($arrTipos[$p]['id_tipo_archivo']==$data['id_tipo']) ? "true":"false" ?>" 
                aria-controls="panel--<?=($p+1)?>" 
                tabindex="0" 
                onclick="xsl.ui.tab(this, event)">
                <span><?=$arrTipos[$p]['tiparc_nombre']?></span></button>
            </a>
            <?php } 
              }
            ?>
          ...
          <div id="panel--1" role="tabpanel" tabindex="0" aria-labelledby="tab-lastnews-2">
            ...
            <div class="pannel-table-wwf" role="tabpanel" tabindex="0" aria-labelledby="tab-lastnews-1">
              <div class="pannel-table-wwf--header">
                ...
                <form method="get" action="<?= base_url() ?>/archivoscampo/search">
                ...
                <table class="table-pro display" style="width:100%">
                  <thead>
                    <tr>
                      ...
                      <th style="text-align: center;" width="40%">ARCHIVO-CAMPO</th>
                  ...
                  <tbody>
                    ...
                    ?>
                      <tr>
                        <td style="text-align: center;"><?=$arrRegistros[$p]['arccam_nombre']?></td>
                        <td style="text-align: center;"  class="text-center">
                            <?php 
                            if ($arrRegistros[$p]['tiparc_nombre']=='FOTOS') { 
                            ?>
                             <img src="<?=$arrRegistros[$p]['arccam_archivo']?>" style="max-width: 50%;" />
                            <?php 
                            } 
                            else{
                            ?>
                            <video src="<?=$arrRegistros[$p]['arccam_archivo']?>" style="max-width: 50%;" controls type="video/mp4"></video>
                            <?php 
                            }?>
                        </td>
                        <td style="text-align: center;" class="text-center">
                          <a href="<?=$arrRegistros[$p]['arccam_archivo']?>" target="_blank"><button class="btn-sm">
              ...
              <div class="footer-pagination">
                ...
                <div class="pagination-datatable">
                  ...
                  href="<?= ($data['pagina']==1) ? "#" :  base_url()."/archivoscampo/search?s=".$data['busqueda']."&id_tipo=".$data['id_tipo']."&p=".($data['pagina']-1) ?>"> <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.5 11L6.5 8L9.5 5" stroke="#868FA0" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></a>

                  <div class="total-pages"><?= $data['pagina'] ?> / <?= $data['total_paginas'] ?></div><a class="button-small-arrow arrow-right" 
                  href="<?= base_url() ?>/archivoscampo/search?s=<?= $data['busqueda']?>&id_tipo=<?= $data['id_tipo']?>&p=<?= ($data['pagina'] < $data['total_paginas']) ? $data['pagina']+1 : $data['total_paginas'] ?>"><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6.5 11L9.5 8L6.5 5" stroke="#464F60" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
                </div>
              </div>
            </div>
